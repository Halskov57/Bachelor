...
const LoginBox: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (mode === 'login') {
      try {
        const res = await fetch(process.env.REACT_APP_API_URL || 'http://localhost:8081/users/verify', {
          method: 'POST',
      ...
      try {
        ...
        const res = await fetch(process.env.REACT_APP_API_URL || 'http://localhost:8081/users/create', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ username, password }),
        });
